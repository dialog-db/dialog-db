...
Two measurement traps worth recording. The unshaped profile's demand shape differs from broadband's *legitimately* (probe 3 vs 2 requests, filtered 151 vs 109 blocks): warming replaces the fold's per-row spilled-value demand faults with range walks, so speculation changes not just when blocks arrive but which reads exist. Regression attribution must compare same profile, same commit — a `none` table is not a demand baseline for a broadband table. And one early stage-2 broadband table (filtered 8.4s / 105 rounds) turned out to be a binary built from an intermediate tree: it also shifted deterministic phases that precede any speculation (probe 2→3, claim puts 1→3), which is the tell to check before chasing a phantom.
