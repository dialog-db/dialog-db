...
3. **Attribution reasons over the transitive tracked set, which never forgets.** By-reference content's provenance is the union of remotes reachable through the upstream entries, local entries resolved through their own branches. Sound today because tracking entries are append-only; if entry removal ever ships, the fast path must weaken to "sole remote *ever* tracked" or fall back to probes.
