...
Two assumptions worth keeping explicit. **Attribution requires the tracked set to never forget**: it reasons over currently-tracked remotes, while by-reference content is a historical accumulation — sound today because tracking entries are append-only, but if entry removal ever ships, the fast path must weaken to "sole remote *ever* tracked" or fall back to probes. **Probe trust requires protocol-written stores**: presence implying subtree presence rests on children-before-parents writes, so a remote restored from a partial backup can hold orphan parents that make a probe over-trust — restore stores atomically or not at all. A *replaced* remote (same address, different store) is guarded upstream of both: its head cannot equal the recorded sync base, so push refuses as non-fast-forward before either assumption fires, and re-anchoring happens through pull. The swap-to-empty case wedges at non-fast-forward rather than dangling — pull from an empty remote is a scenario-1 no-op and never resets the base — and recovery belongs to the genesis/reset flow, not sync.
