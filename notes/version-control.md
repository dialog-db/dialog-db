...
- **Pull** is the scenario cascade above. A branch tracks several upstreams, each with its own sync base; `pull().from(target)` pulls any branch, recording its base from then on. That recording is bookkeeping, not a decision: the entry is flagged tracking-only, so it never becomes what a *bare* pull or push targets. Only `set_upstream` sets that.
...
A branch is its cells, so `branch(name).delete()` removes them — head, tracking state, induction watermark — and the branch stops existing. Revisions are untouched: they are content-addressed blocks reachable from any head that descends from them, so a branch merged elsewhere first loses nothing, and one that was not loses the only reference to its work. Re-creating the name starts from genesis rather than resurrecting the old head, which is what makes this a removal and not the rewind [reset](#branches-and-day-to-day-sync) forbids. The cells go one compare-and-set at a time, so an interrupted delete leaves the branch partly gone and a retry finishes it.

What a delete cannot clean up is another branch's tracking entry naming it — the set is append-only (invariant 3), and nothing can enumerate who tracks whom. Such an entry stops resolving, which attribution reads as unknown provenance: probes decide instead of the zero-request fast path. When the deleted branch was the only route to a remote holding content the survivor carries by reference, there is no source to forward from and the push refuses rather than publishing a head its readers could not serve (`it_refuses_to_publish_a_head_whose_only_provenance_was_deleted`). Keep the remotes a branch depends on reachable from that branch, and deleting the branch it merged from costs nothing.
...
- **Head side**: a block this replica does not hold is not its novelty (a replica stores what it mints), but whether the *target* holds it needs adjudication. The walk settles the node, records it on the **by-reference frontier** (`TreeDifference::unresolved_target`), and push adjudicates per frontier root, cheapest evidence first:
  1. **Attribution** — when the target is the only remote reachable from the tracked upstream set, everything by-reference came from it. Zero requests. Reachable is **transitive through local upstreams**: a local upstream shares this archive, but its head can hold content by reference from remotes only *it* tracks, so attribution walks `Upstream::Local` entries into their branches' own tracked sets (`tracked_remote_names`). Stopping at the local entry launders that content's provenance and silently skips forwarding it (pinned by `it_forwards_content_adopted_through_a_local_upstream`).
  2. **Existence probe** — one forked get per frontier root; a hit prunes the entire subtree. Sound because of the reference-order writes below: presence on a protocol-written store implies the presence of everything referenced. A virgin target (no revision) skips probes outright.
...
**Writes land in reference order.** A push transfers, in order: blob bytes and spilled value blocks; then by-reference frontier subtrees (post-order, each node's blobs at entry); then held novelty children-before-parents (waves with barriers); then the revision. The order is chosen so that at every instant — including the instant an aborted push dies — every block on the target has its full closure (children, spilled values, blob bytes) already durable. This is stronger than "the published head is complete": unpublished residue from an aborted push is unreachable through any head, but it is NOT invisible to a later pusher's existence probes, and a probe that hits residue prunes a whole subtree on the strength of it. Node-before-blob or parent-before-frontier-child orderings leave exactly such poisoned residue (pinned by `it_leaves_an_aborted_push_closure_complete` and `it_leaves_an_aborted_bridge_push_closure_complete`).

### Invariants

The partial-replica sync model rests on these; each is enforced or pinned somewhere named, and a change that weakens one must revisit every consumer listed with it.

1. **A replica stores every block it mints.** Absence therefore means "not my novelty", never "lost my novelty". This is what makes `MissingBlocks::Boundary` sound on the head side of the push diff. It silently forbids evicting or garbage-collecting locally minted blocks that have not been pushed; if pruning ever ships, it needs a pushed-watermark floor.
2. **The recorded sync base is target-known.** A branch's sync base for an upstream is only ever set from heads that upstream served or accepted, so an absent base-side block during the push diff is definitionally present on the target. Base-side `Boundary` is sound at any remote count because losing subtraction only over-uploads, and content-addressed puts are idempotent.
3. **Attribution reasons over the transitive tracked set, which never forgets.** By-reference content's provenance is the union of remotes reachable through the upstream entries, local entries resolved through their own branches. Sound because tracking entries are append-only: nothing removes one, and a sync against an explicitly named target records its base as a *tracking-only* entry rather than leaving the set untouched (which is also why the default upstream is the first entry not so flagged, not simply the first entry — recording a base must not decide what a bare pull or push targets). A local entry can still stop resolving, because [a branch can be deleted](#deleting-a-branch): that is read as provenance *unknown*, never as "no remotes through there", so the fast path stands down and probes decide (`it_forwards_adopted_content_when_a_tracked_local_upstream_is_gone`). If a *remote* entry's removal ever ships, the fast path must weaken to "sole remote *ever* tracked" or fall back to probes unconditionally.
4. **Probe trust: presence implies closure.** A block present on a protocol-written store has every block and blob it references present too, because every writer lands bytes in reference order (invariant 5) — so one positive probe settles a whole subtree, blobs included. A store written outside the protocol (a partial backup restore) can violate this and make probes over-trust; restore stores atomically or not at all. A *replaced* remote (same address, different store) is guarded upstream: its head cannot equal the recorded sync base, so push refuses as non-fast-forward before probe trust fires, and re-anchoring happens through pull. The swap-to-empty case wedges at non-fast-forward rather than dangling — pull from an empty remote is a scenario-1 no-op and never resets the base — and recovery belongs to the genesis/reset flow, not sync.
5. **Every prefix of a push leaves the target closure-complete.** The reference-order writes above; this is what invariant 4 stands on. Pinned by the aborted-push closure tests.
6. **Publish is last.** The revision lands only after every block and blob it references, so a *published* head is always fully served — this bounds the blast radius of every failure above to "retry the push", never "readers break".
7. **A read that misses names its cause.** On a partial replica a local miss is routine, so the miss path is load-bearing: a tracked remote that fails to *load* is carried into the store as `RemoteFallback::Unavailable`, and the first read that actually needed it fails naming the remote and the load failure, instead of silently degrading to local-only and surfacing later as a bare "Blob not found" (pinned by `it_fails_a_miss_loudly_when_the_tracked_remote_is_unavailable`). Reads the local archive satisfies never pay for the remote's unavailability.

Worth remembering why these are written down at all. Before the hitchhiker buffers landed, the system survived partial replicas by an *unstated* theorem: a canonical copy-on-write commit read (and, through the networked store's write-back, hydrated) every spine in every index region it touched, so the push diff's read set was covered by the commit's read set by construction. The buffered write path kept only the EAV slot read (supersession and version absorption) and made the AEV/VAE/history writes blind appends, shrinking commit-side hydration to one region in four while the diff still descends all of them — and hydration itself was always conditional on a remote that loads, with the failure silently swallowed. The invariants above replace that accident of the edit algorithm with an explicit contract that does not depend on hydration at all.
