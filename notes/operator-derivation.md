...
Both were also checked against a real WebKit build (Safari 26 /
AppleWebKit 605.1.15, via Playwright) with
`scripts/webkit-derivation-check`, which reimplements the browser half in
plain WebCrypto and compares it to the Rust fixtures. WebKit computes the
same shared secret and the same derived bytes as Rust and as Chromium,
stably across runs — and it reaches them through a *different archive
shape*, since WebKit cannot structured-clone an X25519 `CryptoKey`
(`TypeError: Unable to deserialize data`) and so must archive the
agreement key AES-KW wrapped. `secret::web_tests` forces that same path
on Chromium, so CI covers it.
