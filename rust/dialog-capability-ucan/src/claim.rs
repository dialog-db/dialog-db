...
use dialog_capability::{Ability, Capability, Constraint, Did, Policy, Provider};
...
mod tests {
    ...
    mod test_provider {
        ...
        use dialog_capability::storage::{GetCapability, ListCapability, SetCapability};
    ...
    async fn claim_self_authorized() {
        ...
        let authority = dialog_capability::Subject::from(subject_did.clone())
        ...
        let cap = dialog_capability::Subject::from(subject_did).invoke(authority::Identify);
    ...
    async fn find_chain_with_parameters_to_policy() {
        ...
        let cap = dialog_capability::Subject::from(subject_did.clone())
            .attenuate(Storage)
        ...
        let matching_cap = dialog_capability::Subject::from(subject_did.clone())
            .attenuate(Storage)
            .attenuate(Store::new("index"))
            .invoke(storage::Get::new(b"some-key"));

        let matching_args = crate::parameters::parameters(&matching_cap);
        let chain = find_chain(
            &env,
            &operator_did,
            &operator_did,
        ...
        let wrong_cap = dialog_capability::Subject::from(subject_did.clone())
            .attenuate(Storage)
            .attenuate(Store::new("secret"))
            .invoke(storage::Get::new(b"some-key"));

        let wrong_args = crate::parameters::parameters(&wrong_cap);
        let chain = find_chain(
            &env,
            &operator_did,
            &operator_did,
