...
fn generate_site(input: &DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
        ...
        .map(|f| {
            ...
            quote! {
                ...
                #address_enum_name::#vname(addr) => {
                    Self::#vname(::dialog_capability::Fork::new(capability, addr).into())
                }
            }
        })
        .collect();
    ...
    let authorize_bounds: Vec<_> = fork_alias_names
        ...
        .map(|(alias, site_alias)| {
            quote! {
                #alias<Fx>: ::dialog_capability::SiteFork<
    ...
    Ok(quote! {
        ...
        impl<Fx: ::dialog_capability::Effect> ::core::convert::From<
            ::dialog_capability::Fork<#struct_name, Fx>,
        > for #fork_enum_name<Fx> {
            fn from(
                fork: ::dialog_capability::Fork<#struct_name, Fx>,
            ) -> Self {
        ...
        impl<Fx, Env> ::dialog_capability::SiteFork<Env> for #fork_enum_name<Fx>
        where
