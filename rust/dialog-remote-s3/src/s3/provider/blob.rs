    ...
    async fn execute(&self, input: S3Invocation<Read>) -> Result<BlobReader, BlobError> {
        ...
        if let Some(range) = input.capability.range() {
            let value = match range.length {
                Some(length) => {
                    format!("bytes={}-{}", range.offset, range.offset + length.max(1) - 1)
                }
...
impl S3BlobSource {
    fn from_response(response: reqwest::Response) -> Self {
        ...
        let stream: ByteStream = Box::pin(
            response
                .bytes_stream()
                .map(|chunk| chunk.map(|b| b.to_vec()).map_err(|e| BlobError::Io(e.to_string()))),
        );
        #[cfg(all(target_arch = "wasm32", target_os = "unknown"))]
...
mod tests {
    ...
    async fn it_imports_then_reads_a_blob_over_s3() -> anyhow::Result<()> {
        ...
        let missing = subject
            ...
            .perform(&network)
            .await;
        assert!(matches!(missing, Err(dialog_effects::blob::BlobError::NotFound(_))));
