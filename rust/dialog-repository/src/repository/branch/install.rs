...
use dialog_artifacts::{BlobIndexExt as _, ENTITY_KEY_TAG, HISTORY_KEY_TAG, Key, State};
...
use crate::{
    ...
    RepositoryArchiveExt as _, RepositoryMemoryExt as _, Revision,
};
...
/// reachable tree, live spilled-value blocks, and referenced blobs. It does not
/// publish a branch head, create a commit, or otherwise make the destination
/// branch visible.
...
impl InstallRevision<'_> {
        ...
        Destination: Provider<Put> + Provider<BlobImport> + ConditionalSync + 'static,
    {
        ...
        let remote = match branch.upstreams().remote_name() {
            Some(name) => Some(
                branch
                    .subject()
        ...
        let scan_tree = current.clone();
        let entries = scan_tree.stream_range(Key::min()..=Key::max(), &tree_storage);
        futures_util::pin_mut!(entries);
        while let Some(entry) = entries.next().await {
            let entry = entry?;
            if !matches!(entry.value, State::Added(_))
                || !matches!(entry.key.tag(), ENTITY_KEY_TAG | HISTORY_KEY_TAG)
            {
                continue;
            }
            let Some(reference) = entry.key.value_spill_hash() else {
                continue;
            };
            let reference: [u8; 32] = reference
                .try_into()
                .map_err(|_| InstallRevisionError::InvalidSpillReference(reference.to_vec()))?;
            spills.insert(reference);
        }

        for reference in spills {
            let expected = NodeHash::from(reference);
            let bytes = source_index
                .get(&reference)
                .await?
                .ok_or_else(|| InstallRevisionError::MissingArchiveBlock(expected.clone()))?;
            if !expected.matches(&bytes) {
                return Err(dialog_storage::DialogStorageError::Verification(
                    "Retrieved spilled-value bytes did not match their content reference"
                        .to_string(),
                )
                .into());
            }
...
mod tests {
    ...
    async fn prepare_stage() -> Result<Stage> {
        ...
        let large = Value::String(
            "staged".repeat(dialog_search_tree::Manifest::default().inline_n as usize + 1),
        );
            ...
            + 'static,
    {
        ...
        let history = destination_branch.log(destination, 10).await?;
        assert!(!history.is_empty(), "revision history remains readable");
        Ok(())
    }
