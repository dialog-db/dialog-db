...
/// Build a `Capability<Put>` for the given catalog + content. The digest is
/// content-addressed: it's derived from the bytes, so the caller doesn't
/// supply one.
fn build_put(
    subject: Subject,
    catalog: &str,
