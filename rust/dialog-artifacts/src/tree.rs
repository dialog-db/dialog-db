...
//!
//! Both [`Artifacts`](crate::Artifacts) and the higher-level branch / layer
...
//! they live here as pluggable building blocks parameterized over any
//! [`ContentAddressedStorage<Hash=Blake3Hash, Error=DialogStorageError>`].
//!
//! Callers responsible for revisions, upstreams, remote fallback, or any
//! other branch/layer specifics keep that logic on their side and call
//! these helpers for the actual key writes and range scans.
...
/// Drain a stream of [`Instruction`]s into `tree`, applying the same key
/// writes that a branch commit or `Artifacts::commit` would.
///
/// Each instruction touches all three EAV/AEV/VAE indexes; `Replace`
/// additionally scans the `(entity, attribute)` range to supersede any
/// different-valued priors (and skips inserting when a same-valued prior
/// is already in place — that's the cardinality-one no-op).
///
/// Callers own everything else: building the change stream, choosing a
/// base tree root, persisting a `Revision`, etc.
        ...
        match instruction {
            ...
            Instruction::Replace(artifact) => {
                ...
                for key in superseded_keys {
                    ...
                    tree.delete(&entity_key.into_key(), store).await?;
                    tree.delete(&value_key.into_key(), store).await?;
                    tree.delete(&attribute_key.into_key(), store).await?;
                }
...
/// Scan the tree for [`Artifact`]s matching the given constrained selector.
///
/// Picks the EAV/AEV/VAE index based on which field of the selector is
/// constrained (entity / value / attribute, in that priority order), then
/// streams the matching key range. Items in the range that don't fully
/// satisfy the selector and items in the `Removed` state are filtered out.
///
/// `tree` is cloned to pin the root; `store` is the
/// [`ContentAddressedStorage`] backing it.
pub fn scan<'s, S>(
    tree: ArtifactTree,
    store: S,
