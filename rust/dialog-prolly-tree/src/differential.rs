...
/// Represents nodes in a "novel" sub-tree that don't exist in a "base" tree.
///
/// Unlike [`Delta`] which expands all the way to leaf entries to compute changes,
/// [`Novelty`] identifies which tree **nodes** (at any level) are new and need to
/// be replicated. When a node is found to be entirely novel (no corresponding node
/// in base), we yield the entire subtree without expanding it further.
///
/// This is useful for sync/replication scenarios where you need to push all nodes
/// that a remote doesn't have, without caring about the specific entry-level changes.
///
/// # Example
///
/// ```ignore
/// // Compute which nodes in `local_tree` don't exist in `base_tree`
/// let mut novelty = Novelty::new(&base_tree, &local_tree);
/// novelty.expand().await?;
///
/// // Stream all novel nodes (including entire subtrees that are new)
/// let nodes = novelty.stream();
/// pin_mut!(nodes);
/// while let Some(node) = nodes.next().await {
///     let node = node?;
///     // Push node.hash() to remote storage
/// }
/// ```
pub struct Novelty<'a, Key, Value, Hash, Storage>
where
    Key: KeyType + 'static,
    Value: ValueType,
    Hash: HashType,
    Storage: ContentAddressedStorage<Hash = Hash>,
{
    /// Nodes from the base tree (what remote already has)
    base: SparseTree<'a, Key, Value, Hash, Storage>,
    /// Nodes from the novel tree (what we have locally)
    novel: SparseTree<'a, Key, Value, Hash, Storage>,
}

impl<'a, Key, Value, Hash, Storage> Novelty<'a, Key, Value, Hash, Storage>
where
    Key: KeyType + 'static,
    Value: ValueType,
    Hash: HashType,
    Storage: ContentAddressedStorage<Hash = Hash>,
{
    /// Creates a new Novelty from a base tree and a novel tree.
    ///
    /// After calling `expand()`, this will identify nodes in `novel` that
    /// don't exist in `base`.
    pub fn new<Distribution: crate::Distribution<Key, Hash>>(
        base: &'a Tree<Distribution, Key, Value, Hash, Storage>,
        novel: &'a Tree<Distribution, Key, Value, Hash, Storage>,
    ) -> Self {
        Self {
            base: SparseTree {
    ...
    /// Expands nodes to identify which nodes in `novel` don't exist in `base`.
    ///
    /// After this completes, `novel` will contain only nodes that need to be
    /// replicated - either because they're entirely new, or because they're
    /// at a point where the trees diverge.
    ///
    /// ## Algorithm
    ///
    /// The algorithm is similar to [`Delta::expand`] but with a key difference:
    /// - When a node in `novel` has no corresponding boundary in `base`, we **keep**
    ///   it without expanding (the entire subtree is new)
    /// - When boundaries match but hashes differ, we expand both to find where they diverge
    /// - When boundaries and hashes match, we prune (node already exists in base)
    pub async fn expand(&mut self) -> Result<(), DialogProllyTreeError> {
        use std::cmp::Ordering;

        loop {
            // First, prune any nodes with matching boundaries and hashes
            self.novel.prune(&mut self.base);
            ...
            while novel_idx < self.novel.nodes.len() && base_idx < self.base.nodes.len() {
                let novel_bound = self.novel.nodes[novel_idx].upper_bound().clone();
                let base_bound = self.base.nodes[base_idx].upper_bound().clone();

                match novel_bound.cmp(&base_bound) {
                    Ordering::Less => {
                        // Novel node has smaller boundary - it might be entirely new,
                        // OR base might have a larger node covering this range.
                        // Expand base to check if it contains something matching novel.
                        if self
                            .base
                            .expand(base_bound.clone()..=base_bound.clone())
                            .await?
                        {
                            expanded = true;
                            break; // Restart comparison after expansion
                        }
                        // Base couldn't be expanded further, so novel node is unique
                        novel_idx += 1;
                    }
                    Ordering::Greater => {
                        // Base node has smaller boundary - novel might have a larger
                        // node covering this range. Expand novel to check.
                        if self
                            .novel
                            .expand(novel_bound.clone()..=novel_bound.clone())
                            .await?
                        {
                            expanded = true;
                            break; // Restart comparison after expansion
                        }
                        // Novel couldn't be expanded, move base forward
                        base_idx += 1;
                    }
                    Ordering::Equal => {
                        // Same boundary - check if hashes differ
                        if self.novel.nodes[novel_idx].hash() != self.base.nodes[base_idx].hash() {
                            // Different hashes - need to expand both to find divergence
                            if self
                                .novel
                                .expand(novel_bound.clone()..=novel_bound.clone())
                                .await?
                        ...
                        // If hashes match, prune will have removed them
                        novel_idx += 1;
                        base_idx += 1;
                    }
                }
    ...
    /// Returns a stream of all novel nodes.
    ///
    /// This yields every node that needs to be replicated, including all nodes
    /// in subtrees that are entirely new. For each novel node, we recursively
    /// yield all its descendants.
    pub fn stream(
        &self,
    ) -> impl Stream<Item = Result<Node<Key, Value, Hash>, DialogProllyTreeError>> + '_ {
        try_stream! {
            for sparse_node in &self.novel.nodes {
                // Load the node
                let node = sparse_node.clone().load(self.novel.storage).await?;
        ...
        try_stream! {
            if let Ok(refs) = node.references() {
                for reference in refs.iter() {
                    // Load child node
                    let child = Node::from_reference(reference.clone(), self.novel.storage).await?;
