...
//! ordering choice for the planner — this bench drives a two-premise
//! `DeductiveRule` (`employee_from_stuff`): an `Employee` is derived by
//! joining `stuff/name` and `stuff/role` on a shared entity. Evaluating
//! that rule issues many `Provider<Select>` calls (per premise, and once
//! per outer binding for the inner premise); every one routes through a
//! `CountingStore` over a single shared `ReadJournal`, so the headline
//! metric — total block fetches across the *whole* planned query — is the
...
fn bench_query_join(c: &mut Criterion) {
    ...
    for &size in SIZES {
        ...
        let run = rt.block_on(env.query_employees()).unwrap();
        println!(
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, _| {
            b.to_async(runtime())
                .iter(|| async { env.query_employees().await.unwrap() });
        });
    }
