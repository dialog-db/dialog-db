...
use dialog_query::concept::{Concept, Match as ConceptMatch, Attributes, Instance, Instructions};
use dialog_query::fact_selector::FactSelector;
use dialog_query::rule::{Rule, When, Statements};
...
impl Rule for Person {
    fn when(terms: Self::Match) -> When {
        ...
        let name_selector = FactSelector::<Value> {
            the: Some(Term::from("person/name".parse::<dialog_artifacts::Attribute>().unwrap())),
        ...
        let age_selector = FactSelector::<Value> {
            the: Some(Term::from("person/age".parse::<dialog_artifacts::Attribute>().unwrap())), 
            of: Some(terms.this.clone()),
...
mod tests {
    ...
    fn test_person_match_creation() {
        ...
        assert_eq!(name_query.the.as_ref().map(|t| t.as_constant().cloned()).flatten(), 
                   Some("person/name".parse::<dialog_artifacts::Attribute>().unwrap()));
        assert!(name_query.is.is_some());
        
        let age_query = attributes.age.is(30u32);
        assert_eq!(age_query.the.as_ref().map(|t| t.as_constant().cloned()).flatten(),
