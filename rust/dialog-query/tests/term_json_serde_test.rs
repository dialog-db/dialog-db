...
fn test_correct_json_formats() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test Any: should be { "?": {} }
    let any_term: Term<Value> = Term::default();
...
fn test_term_any_json_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test deserialization
    let parsed_any: Term<Value> = serde_json::from_str(r#"{"?":{}}"#)?;
    assert!(parsed_any.is_blank());
...
fn test_mixed_terms_in_fact_selector() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        fact_selector.the.as_ref().unwrap().name().unwrap(),
        "attribute"
    );

    // Check 'of' is Any
    assert!(fact_selector.of.as_ref().unwrap().is_blank());
