...
impl Context {
    ...
    /// Resolve a [`Type`] to its current form under the
    /// substitution, recursively following any variables that have
    /// been bound until reaching a static type or an unbound
    /// variable.
...
mod tests {
    ...
    fn unify_variable_constraint_propagates() {
        ...
        let comparable = Type::Static(StaticType::from(Primitive::COMPARABLE));
        ctx.unify(&v_ty, &comparable).unwrap();
        let resolved = ctx.resolve(&v_ty);
        match resolved {
    ...
    fn chained_unification_propagates() {
        ...
        ctx.unify(&Type::Variable(t), &p(ValueType::UnsignedInt))
            .unwrap();
        let t_resolved = ctx.resolve(&Type::Variable(t));
        let u_resolved = ctx.resolve(&Type::Variable(u));
