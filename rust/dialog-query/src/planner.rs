...
use crate::error::{Infeasible, TypeError};
...
impl Planner {
    ...
    pub fn plan(mut self, scope: &Environment) -> Result<Conjunction, TypeError> {
        ...
        while !remaining.is_empty() {
            ...
            for (index, premise) in remaining.iter().enumerate() {
                let Ok(binds) = premise.feasible(&bound) else {
                    continue;
                };
    ...
    /// Rewrite the planner's premises so each variable term carries
    /// its rule-level inferred kind.
    ///
    /// The type environment is *taken*, not recomputed: when the
    /// caller built the planner with [`with_types`](Self::with_types)
    /// (the path `DeductiveRule::plan` always uses), the analysis env
    /// is reused as-is, so repeated planning of the same rule across
    /// scopes never re-runs inference. Inference happens here only on
    /// the [`From<Vec<Premise>>`] path (no prior analysis), and even
    /// then just once: `take()` leaves `types` as `None`, but a
    /// `Planner` is consumed by a single `plan` call, so `narrow`
    /// runs at most once per planner.
    ...
    fn fail(remaining: &[Premise], environment: &Environment) -> Result<Conjunction, TypeError> {
        for premise in remaining {
            if let Err(Infeasible::NeedsAll(needs)) = premise.feasible(environment)
                && !needs.is_empty()
            {
...
mod plan_ordering {
    ...
    use crate::the;
    use crate::{
        AttributeDescriptor, Cardinality, ConceptDescriptor, Parameters, Proposition, Term, Type,
    };
    use dialog_artifacts::Entity;
