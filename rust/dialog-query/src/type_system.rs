...
///
/// Three constraints exist: a lexical prefix over the TEXTUAL
/// kinds, a conformance set over Entity — the value must satisfy a
/// concept-typed field's target concepts — and a name shape over
/// Symbol — the attribute's name half must be a symbol or a
/// position. Numeric intervals extend this struct the same way
/// rather than adding lattice variants.
///
/// Invariant: never empty (no prefix, no conformance and no name
/// shape is no refinement; the constructors collapse it to an
/// unrefined type).
...
impl Refinement {
    ...
    fn is_empty(&self) -> bool {
        self.prefix.is_none() && self.conforms.is_empty() && self.name_shape.is_none()
    }
    ...
    /// name shapes must agree (an attribute name is a symbol or a
    /// position, never both, so conflicting shapes admit nothing).
    ...
    fn implied_by(&self, other: &Refinement) -> bool {
        ...
        prefix_implied && shape_implied && self.conforms.is_subset(&other.conforms)
    }
