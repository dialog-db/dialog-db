...
impl Type {
    ...
    pub fn without_nothing(self) -> Type {
        match self {
            ...
            Type::Refined(p, r) => Type::Refined(p.without_nothing(), r),
        }
    }
    ...
    pub fn with_prefix(self, prefix: impl Into<String>) -> Option<Type> {
        ...
        if membership.without_nothing().is_empty() {
            return None;
        }
...
mod tests {
    ...
    fn with_prefix_narrows_to_textual() {
        let refined = Type::primitive_set(Primitive::ALL)
            .with_prefix("did:")
        ...
        assert_eq!(
            Type::primitive(ValueType::String).with_prefix(""),
            Some(Type::primitive(ValueType::String)),
    ...
    fn refined_intersect_takes_the_stronger_prefix() {
        let did = Type::primitive(ValueType::Entity)
            .with_prefix("did:")
            .unwrap();
        let did_key = Type::primitive(ValueType::Entity)
            .with_prefix("did:key:")
            .unwrap();
        let met = did.intersect(&did_key).expect("compatible prefixes");
        assert_eq!(met.refinement().unwrap().prefix, "did:key:");

        let http = Type::primitive(ValueType::Entity)
            .with_prefix("http:")
            .unwrap();
        assert!(
        ...
        let unrefined = Type::primitive_set(Primitive::TEXTUAL);
    ...
    fn refined_union_weakens() {
        let a = Type::primitive(ValueType::String)
            .with_prefix("user/admin")
            .unwrap();
        let b = Type::primitive(ValueType::String)
            .with_prefix("user/guest")
            .unwrap();
        let joined = a.union(&b);
        assert_eq!(joined.refinement().unwrap().prefix, "user/");

        let unrefined = Type::primitive(ValueType::String);
        assert_eq!(
            a.union(&unrefined),
        ...
        let disjoint = Type::primitive(ValueType::String)
            .with_prefix("group/")
            .unwrap();
        assert!(
    ...
    fn refined_includes_is_constraint_ordered() {
        ...
        let did_key = Type::primitive(ValueType::Entity)
            .with_prefix("did:key:")
            .unwrap();
        let unrefined = Type::primitive(ValueType::Entity);
    ...
    fn refined_admits_checks_lexical_form() {
        let refined = Type::primitive_set(Primitive::TEXTUAL)
            .with_prefix("user/")
            .unwrap();
    ...
    fn refined_serde_round_trip() {
        let t = Type::primitive(ValueType::Entity)
            .with_prefix("did:key:")
            .unwrap();
        let j = serde_json::to_string(&t).unwrap();
        let back: Type = serde_json::from_str(&j).unwrap();
