    ...
    pub fn evaluate<M: Selection>(self, selection: M) -> impl Selection {
        match self {
            Constraint::Equality(c) => {
                Either::Left(Either::Left(Either::Left(c.evaluate(selection))))
            }
            Constraint::Coalesce(c) => {
                Either::Left(Either::Left(Either::Right(c.evaluate(selection))))
            }
            Constraint::TypeOf(c) => {
                Either::Left(Either::Right(Either::Left(c.evaluate(selection))))
            }
            Constraint::StartsWith(c) => {
                Either::Left(Either::Right(Either::Right(c.evaluate(selection))))
            }
            Constraint::LessThan(c) => {
                Either::Right(Either::Left(Either::Left(c.evaluate(selection))))
            }
            Constraint::AtMost(c) => {
                Either::Right(Either::Left(Either::Right(c.evaluate(selection))))
            }
            Constraint::GreaterThan(c) => {
                Either::Right(Either::Right(Either::Left(c.evaluate(selection))))
            }
            Constraint::AtLeast(c) => {
                Either::Right(Either::Right(Either::Right(c.evaluate(selection))))
            }
        }
    }
}
