...
use crate::variable::{Variable, Untyped};
...
impl Term {
    ...
    pub fn as_variable(&self) -> Option<&Variable<Untyped>> {
        match self {
...
impl<T> From<Variable<T>> for Term {
    fn from(var: Variable<T>) -> Self {
        // Convert any Variable<T> to Variable<Untyped> for Term
        Term::Variable(var.as_untyped())
    }
}
...
mod tests {
    ...
    fn test_turbofish_syntax_with_terms() {
        ...
        let any_var = Variable::untyped("wildcard");
