...
impl Coalesce {
    ...
    /// kind, and `is`'s kind.
    ///
    /// **A non-optional source is rejected, on purpose.** If
    /// `source` carries a static kind, it must be set-widened (admit
    /// `Nothing`); a source that can never be `Absent` is rejected
    /// with [`UnifyError::SourceNotOptional`].
    ///
    /// This is not a soundness requirement: feeding a never-absent
    /// `T` into something that accepts `Optional<T>` is type-safe,
    /// the fallback path would simply never run. We reject it anyway
    /// because such a coalesce is *redundant, and redundancy here
    /// signals a mistake*. `Coalesce` exists only to supply a value
    /// for an absent one; if the source can't be absent, the
    /// fallback is dead code the author wrote believing the source
    /// was optional. Failing at compile time surfaces that
    /// misunderstanding, where silently treating it as a no-op would
    /// let the logic error through to manifest later as "my fallback
    /// never applies." For example:
    ///
    /// ```text
    /// // `name` is a required (always-present) field, so `?name`
    /// // has a non-optional kind. The author likely meant `name`
    /// // to be optional; this is rejected rather than run as a
    /// // no-op that ignores "Anon".
    /// name.unwrap_or("Anon").is(?display)   // SourceNotOptional
    /// ```
    ///
    /// If `source` has no static kind (`kind() == None`), it is
    /// treated as fully unconstrained: the unifier may not narrow
    /// it. The caller is then responsible for ensuring the source is
    ...
    pub fn validate(&self, ctx: &mut Context) -> Result<(), UnifyError> {
        ...
        match self.source.kind() {
            Some(source) if source.is_optional() => {
                // Strip the Nothing bit to get α's required shape.
                let underlying = source.required();
