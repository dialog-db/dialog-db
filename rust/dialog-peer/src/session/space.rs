//! Space capability providers for Session.
...
impl<S> Provider<space_fx::Load> for Session<S>
where
    ...
    Self: ConditionalSend + ConditionalSync,
{
    async fn execute(
        ...
        let location = storage_fx::Location::new(self.peer().directory().clone(), name);
...
#[cfg_attr(not(target_arch = "wasm32"), async_trait::async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait::async_trait(?Send))]
impl<S> Provider<space_fx::Create> for Session<S>
where
    S: Clone + ConditionalSend + ConditionalSync + 'static,
