...
it **deterministic**: the same set of key-value pairs always produces the same
tree, regardless of how they were inserted.
...
Keys with higher ranks cause the tree to branch. This produces a balanced tree
...
The `Artifacts::select()` method chooses the index based on what's constrained
in the selector:
...
These cost levels correspond directly to the planner's cost constants, so the
planner's ordering decisions reflect actual I/O patterns.
